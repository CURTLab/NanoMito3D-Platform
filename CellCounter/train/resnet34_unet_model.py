# code from Nishank Singla, https://github.com/Nishanksingla/UNet-with-ResBlock

import numpy as np
from keras.backend import int_shape
from keras.models import Model
from keras.layers import Conv2D, Cropping2D, MaxPooling2D, Add, BatchNormalization, Input, Activation, UpSampling2D, Concatenate, Dropout, Conv2DTranspose

def res_unet(filter_root, layers, n_class=2, input_size=(256, 256, 1), activation='relu', batch_norm=True, drop_out=True, final_activation='softmax'):
    ...
    for i in range(layers):
        ...
        # Dropout
        if drop_out:
            act1 = Dropout(min(0.3, 0.1 * (i+1)), name=f"Dropout{i}_1")(act1)
    ...
    for i in range(layers - 2, -1, -1):
        ...
        up_conv1 = Conv2DTranspose(out_channel, (2, 2), strides=(2, 2), padding='same', activation='relu', name="conv2DTranspose{}_1".format(i))(x)
        ...
        # Dropout
        if drop_out:
            up_act1 = Dropout(min(0.3, 0.1 * (i+1)), name=f"Dropout{i}_2")(up_act1)
